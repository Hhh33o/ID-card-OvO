...
def iconv(code, newcode, s):
    try:
        if isinstance(s, bytes):
            out = s.decode(code, 'ignore')
        else:
            out = str(s)
        ...
        for i in range(len(coors)):
            ...
            if textIndex == 0:
                ...
                if char == '民' or char == '族':
                    ...
                    if ishan(char.decode('utf-8')):
                        text[1] += char.decode('utf-8')
                elif ishan(char):
                    text[1] += char
...
def CardBirth(BinaryImg, horiBoundaryCoor, origin=None):
    ...
    #         box = np.int64([[newVertiBoundaryCoors[i][0], up],
...
def ocr(imgSrc, lang=None, psm='3'):
    ...
    text = pytesseract.image_to_string(imgSrc, config=custom_config)
    
    return text.strip()


def filterNonnumericChar(s):
    """
    过滤非数字字符
    :param s:
    :return:
